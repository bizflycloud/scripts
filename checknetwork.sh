...
main() {
	# [[ -z $(command -v ping) ]] && echo "ping Not Found, Please install traceroute" && exit 1
	[[ -z $(command -v traceroute ) ]] && echo "Khong tim thay traceroute, ban co muon cai dat traceroute khong? [y/n]"  && read -p "Chon 'y' hoac 'n': " yn &&
	case $yn in
		[Yy]* ) (sudo apt update && sudo apt install traceroute -y || sudo yum update -y && sudo yum install traceroute -y || sudo pacman -Sy && sudo pacman -S traceroute --noconfirm  || sudo dnf update && sudo dnf install traceroute -y || sudo zypper ref && sudo zypper in traceroute) 2>/dev/null ;;
		[Nn]* ) exit 1;;
	esac
	# [[ -z $(command -v tracepath) ]] && echo "tracepath Not Found, Please install tracepath" && exit 1
